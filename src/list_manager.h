...
void calculate_plan_reverse(viaggio *v, int index_partenza, int index_arrivo) {
    ...
    int max_index_curr = get_index_max_raggiungible_station_desh(best_curr_station.index,
    ...
    */
    ...
    while ((curr_index > index_partenza) && (max_index_curr >
                                             index_partenza)/*la seconda e'condizione piu stringente della prima quindi in teoria potrei lasciare solo questa ma devo ancora verificare edge case*/ ) {
        ...
        for (int i = curr_index; i >= max_index_curr; i--) {
            ...
            if (flag_is_changed_best_station == 1) {
                best_station_max_index_ragg = get_index_max_raggiungible_station_desh(best_curr_station.index,
                                                                                      best_curr_station.parco.curr_max.autonomia);
                //.DEBUG
                //printf("\nnew best station found! indice massimo a cui puo arrivare stazione %d: %d; ovvero stazione %d \n", highway.stazioni[i].distanza_da_inizio_autostrada, i_max_ragg_index, highway.stazioni[i_max_ragg_index].distanza_da_inizio_autostrada);
                best_station_next_index = get_next_best_station(best_curr_station.index, best_station_max_index_ragg,
                                                                index_partenza);/*distanza minore ma rientro comunque dentro nella stazione che scegliero come prossima migliore!*/
                flag_is_changed_best_station = 0;
            }

            if (
                    (i_max_ragg_index <=
                     best_station_next_index)/*distanza minore ma rientro comunque dentro nella stazione che scegliero come prossima migliore!*/
                    ||
        ...
        if ((best_curr_station.index ==
             curr_index /*non ce nessuna stazione che riesce a portarmi piu lontano di quella corrente*/) &&
            (/*non riesco a raggiungere arrivo*/get_index_max_raggiungible_station_desh(best_curr_station.index,
                                                                                        best_curr_station.parco.curr_max.autonomia) >
                                                index_partenza)) {
            // Non è stato possibile trovare un percorso
            //.DEBUG
